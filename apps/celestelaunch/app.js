{ // must be inside our own scope here so that when we are unloaded everything disappears
  ...
  let drawPage = function () {
    ...
    if (activeCardSelected == true) { // Draw Submenu
      ...
      g.drawImage(launchCache.apps[activeApp].icon, 72, 15, { scale: 0.7 });
      for (i = 0; i < launchCache.apps.length; i++) {
        let appX;
        if (i > activeApp) {
        ...
        if (i != activeApp) {
          g.drawImage(launchCache.apps[i].icon, appX, 18, { scale: 0.5 });
        }
      }
    } else { //Draw Menu
  ...
  let swipeListener = function (dirLeftRight, dirUpDown) {
    ...
    if (dirLeftRight == 1) {
      if (activeCardSelected == true && activeApp < launchCache.apps.length - 1) {
        activeApp++;
        buzzShort();
  ...
  let touchListener = function (_, p) {
    ...
    if (activeCardSelected) {
      console.log(launchCache.apps[activeApp].src)
      Bangle.load(launchCache.apps[activeApp].src)
    } else {
